...
import { getPujaData, getPujaParts, getSamagriList } from '../services/PujaService';
import { getActiveProfile, type UserProfile } from '../services/ProfileService';

type Props = NativeStackScreenProps<RootStackParamList, 'PujaHome'>;

export default function PujaHomeScreen({ navigation }: Props) {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      ...
      <View style={styles.card}>
           <Text style={styles.descText}>{getPujaData().description}</Text>
      </View>
      ...
      <TouchableOpacity
        ...
        onPress={() => navigation.navigate('Setup')}
      >
        <Text style={styles.actionBtnText}>Setup (Sankalp)</Text>
      ...
      >
        <Text style={styles.actionBtnText}>Samagri — Materials</Text>
           <Text style={styles.actionBtnSub}>
             Checklist of {getSamagriList().length} items needed
           </Text>
      </TouchableOpacity>

      {/* Puja Flow */}
      <Text style={styles.sectionTitle}>Puza Yatra — Puja Journey</Text>
      <Text style={styles.sectionSubtext}>Follow these three stages in order</Text>
         {getPujaParts().map((part, idx) => {
        const phaseLabels: Record<string, { phase: string; icon: string; desc: string }> = {
          A: { phase: 'Aarambh (Beginning)', icon: '🪔', desc: 'Purification, invocations, and setting up the sacred space' },
          B: { phase: 'Archana (Worship)', icon: '🙏', desc: 'Deity worship, mantras, arti, and food offering' },
          C: { phase: 'Visarjan (Closing)', icon: '🎗️', desc: 'Forgiveness prayer and tying the sacred Nariwan' },
        };
        const label = phaseLabels[part.id] || { phase: `Phase ${idx + 1}`, icon: '📖', desc: '' };
        return (
          <TouchableOpacity
            key={part.id}
          ...
          >
            <View style={styles.partLeft}>
              <View style={[styles.partBadge, idx === 0 && styles.partBadgeFirst]}>
                <Text style={styles.partBadgeIcon}>{label.icon}</Text>
              </View>
                 {idx < getPujaParts().length - 1 && <View style={styles.partConnector} />}
            </View>
            <View style={styles.partInfo}>
              <Text style={styles.partPhase}>{label.phase}</Text>
              <Text style={styles.partTitleDev}>{part.title.devanagari}</Text>
              <Text style={styles.partTitleEng}>{part.title.english}</Text>
              <Text style={styles.partDesc}>{label.desc}</Text>
              <Text style={styles.partStepCount}>{part.steps.length} steps</Text>
            </View>
      ...
      >
        ...
        <Text style={styles.actionBtnSub}>Begin from Aarambh — Doop Deep Puja</Text>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  titleEng: { fontSize: 20, fontWeight: '500', color: '#2D2D3A', marginTop: 4 },
  ...
  partBadgeFirst: { backgroundColor: '#6C5CE7' },
  partBadgeIcon: { fontSize: 18 },
  partConnector: {
    width: 2,
    height: 20,
  ...
  partInfo: { flex: 1 },
  partPhase: { fontSize: 12, fontWeight: '600', color: '#6C5CE7', marginBottom: 2 },
  partTitleDev: { fontSize: 16, fontWeight: '600', color: '#9A7B4F' },
  partTitleEng: { fontSize: 14, color: '#2D2D3A', marginTop: 2 },
  partDesc: { fontSize: 12, color: '#888', marginTop: 4, lineHeight: 18 },
  partStepCount: { fontSize: 11, color: '#6C5CE7', fontWeight: '500', marginTop: 4 },
